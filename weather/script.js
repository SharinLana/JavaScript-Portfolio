...
function showResult(resReceived) {
  ...
  temperature.textContent = `${Math.round(
    resReceived.main.temp
  )}°C / ${Math.round(resReceived.main.temp * 1.8 + 32)}°F`;
  ...
  if (resReceived.weather[0].main === "Clear") {
    ...
    resReceived.weather[0].main === "Haze" ||
    resReceived.weather[0].main === "Dust"
  ) {
    snow.style.display = "none";
    document.body.classList.remove("foggy-background");
    document.body.classList.remove("winter-background");
  ...
  min.textContent = `Min: ${Math.round(
    resReceived.main.temp_min
  )}°C / ${Math.round(resReceived.main.temp * 1.8 + 32)}°F`;
  max.textContent = `Max: ${Math.round(
    resReceived.main.temp_max
  )}°C / ${Math.round(resReceived.main.temp * 1.8 + 32)}°F`;
  const mph = 0.621371192;
  if (resReceived.wind.deg >= 23 && resReceived.wind.deg < 68) {
    wind.textContent = `Wind: ${Math.round(
      resReceived.wind.speed
    )} kph / ${Math.round(resReceived.wind.speed * mph)} mph, NE`;
  } else if (resReceived.wind.deg >= 68 && resReceived.wind.deg < 113) {
    wind.textContent = `Wind: ${Math.round(
      resReceived.wind.speed
    )} kph / ${Math.round(resReceived.wind.speed * mph)} mph, E`;
  } else if (resReceived.wind.deg >= 113 && resReceived.wind.deg < 158) {
    wind.textContent = `Wind: ${Math.round(
      resReceived.wind.speed
    )} kph / ${Math.round(resReceived.wind.speed * mph)} mph, SE`;
  } else if (resReceived.wind.deg >= 158 && resReceived.wind.deg < 203) {
    wind.textContent = `Wind: ${Math.round(
      resReceived.wind.speed
    )} kph / ${Math.round(resReceived.wind.speed * mph)} mph, S`;
  } else if (resReceived.wind.deg >= 203 && resReceived.wind.deg < 248) {
    wind.textContent = `Wind: ${Math.round(
      resReceived.wind.speed
    )} kph / ${Math.round(resReceived.wind.speed * mph)} mph, SW`;
  } else if (resReceived.wind.deg >= 248 && resReceived.wind.deg < 293) {
    wind.textContent = `Wind: ${Math.round(
      resReceived.wind.speed
    )} kph / ${Math.round(resReceived.wind.speed * mph)} mph, W`;
  } else if (resReceived.wind.deg >= 293 && resReceived.wind.deg < 338) {
    wind.textContent = `Wind: ${Math.round(
      resReceived.wind.speed
    )} kph / ${Math.round(resReceived.wind.speed * mph)} mph, NW`;
  } else {
    wind.textContent = `Wind: ${Math.round(
      resReceived.wind.speed
    )} kph / ${Math.round(resReceived.wind.speed * mph)} mph, N`;
  }
...
function getDate() {
  ...
  date.textContent =
    `${day} ` + `${todayDate} ` + `${currentMonth} ` + `${year}`;
}

/* Determine the today's date on load  */
